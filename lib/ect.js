...
(function () {
	...
	var	ECT = function (options) {
				...
				for (var i = 0; i < matches.length; i++) {
					...
					if (i % 2 === 1) {
						...
						case '-':
							...
							break;
                        case '%':
                            prefix = '\' + (' + line + '\n\'\') + __ectTemplateContext.translate(';
                            postfix = ') + \'';
                            newline = '';
                            text = text.substr(1);
                            output = 'translated';
							...
							case 'function' :
								...
								switch (indentStack[indentStackPointer - 1]) {
									...
									case 'capture_output_unescaped' :
										...
										buffer += ') ? \'\')';
										break;
                                    case 'capture_output_translated' :
                                        indentStack.pop();
                                        indentStackPointer--;
                                        buffer += ') ? \'\')';
                                        break;
        ...
        TemplateContext.prototype.translate = function (text) {
            if (text == null) {
                return '';
            }

            var result = text.toString();
            if (!ect.options.ts || !this.data.lang || !ect.options.ts[this.data.lang] || !ect.options.ts[this.data.lang].hasOwnProperty(result)) {
                return result;
            }

            return ect.options.ts[this.data.lang][result];
        };
